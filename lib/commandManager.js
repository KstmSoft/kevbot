const _ = require('underscore'),
	tokenizer = require('string-tokenizer'),
...
const handle = async (client, message)=>{
	...
	let request = parseMessage(message);
	...
	try{
		switch(response.type){
			...
			default:
				...
				await client.reply(from, '*[❌ Ha ocurrido un problema]:*\n\nContacta con el Administrador.', message.id);
		}
	}catch(err){
...
function parseMessage(message){
	const body = message.type === 'chat' ? message.body : message.caption;
