...
const handle = async (client, message)=>{
	...
	if(authorized.isAuthorized || authorized.isAdmin){
		...
		switch(response.type){
			...
			case 'sticker':
				client
				.sendImageAsSticker(from, `data:image/webp;base64,${response.content}`, {author: process.env.BOT_NAME, pack: `Created by ${process.env.BOT_NAME}`})
				.catch((erro) => {
				  console.error('Error when sending: ', erro);
				});
				break;
		...
		client.reply(from, `*[No autorizado]:* Para disfrutar de ${process.env.BOT_NAME}, ponte en ` 
							  + `contacto con el Administrador. [+573105523042]`, message.id);
